	...
	public void onStart(Context context) {
		...
		for (LibOpenConnect.FormOpt opt : mForm.opts) {
			...
			case LibOpenConnect.OC_FORM_OPT_TEXT:
				...
				//set default value for password and username for vpn
				//TODO use preference
				OpenConnectHelper helper = new OpenConnectHelper(mContext);
				if (opt.name.equalsIgnoreCase("username"))
					v.addView(newTextBlank(opt, helper.getUsername()));
				else if (opt.name.equalsIgnoreCase("password"))
					v.addView(newTextBlank(opt, helper.getPassword()));
				hasUserOptions = true;
				break;
			case LibOpenConnect.OC_FORM_OPT_SELECT:
